...
public final class SQLHelper {
	...
	public static final boolean isOpen(final Connection connection) {
		try {
			...
			throw new CrosscuttingBodyHealtyException(mensajeTecnico, mensajeUsuario, exception);
	...
	public static final void close(final Connection connection) {
		try {
			if (!isOpen(connection)) {
				...
				throw new CrosscuttingBodyHealtyException(mensajeUsuario, mensajeTecnico);
			}

			connection.close();
		} catch (final CrosscuttingBodyHealtyException exception) {
			throw exception;
		} catch (final SQLException exception) {
			var mensajeUsuario = MessageCatalogStrategy.getContenidoMensaje(CodigoMensaje.M00002);
